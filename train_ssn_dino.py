...
def train(cfg):
    ...
    model = SSN_DINO(cfg.fdim, cfg.nspix, cfg.niter).to(device)
    model.train()
    ...
    while iterations < cfg.train_iter:
        for data in train_loader:
            ...
            if (iterations % cfg.test_interval) == 0:
                ...
                if asa > max_val_asa:
                    max_val_asa = asa
                    torch.save(model.state_dict(), os.path.join(cfg.out_dir, "ssn_dino_finetuned.pth"))
...
if __name__ == "__main__":
    ...
    parser.add_argument("--out_dir", default="./model_checkpoints", type=str, help="/path/to/output directory")
    ...
    parser.add_argument("--train_iter", default=2500, type=int)
