...
def GNN_seg(image_path, plabels, num_spixels_width, num_spixels_height, spixel_list, mode, cut, alpha, epoch, K,
    ...
    # re-init weights and optimizer for every image
    model2.load_state_dict(torch.load('./model2.pt', map_location=torch.device(device), weights_only=True))
    ...
    # re-init weights and optimizer for every image
    model3.load_state_dict(torch.load('./model3.pt', map_location=torch.device(device), weights_only=True))
